...
- **pytest**: gotcha: pytest --basetemp inside a repo/worktree can break walk-up discovery isolation tests because require_mem_dir may find the real parent .projectmem; use an external temp base under %TEMP% for full-suite runs. (from projectmem)
